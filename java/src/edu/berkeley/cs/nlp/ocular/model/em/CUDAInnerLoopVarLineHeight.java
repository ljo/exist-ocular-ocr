 ...
 */
public class CUDAInnerLoopVarLineHeight implements EmissionCacheInnerLoopVarLineHeight {
	...
	public CUDAInnerLoopVarLineHeight(int numThreads, int gpuId) {
...
//		this.cudaModule = CudaUtil.compileAndLoad("/tmp/emission_cache_kernel", kernelSrcPrivate(), true);
	...
	public void startup(float[][] whiteTemplates, float[][] blackTemplates, int[] templateNumIndices, int[] templateIndicesOffsets, int minTemplateWidth, int maxTemplateWidth, int maxSequenceLength, int totalTemplateNumIndices, int lineHeight) {
		...
		cuMemAlloc(d_Ow, (extendedMaxSeqLength+maxTemplateWidth-1)*lineHeight * Sizeof.FLOAT);
		this.d_Ob = new CUdeviceptr();
		cuMemAlloc(d_Ob, (extendedMaxSeqLength+maxTemplateWidth-1)*lineHeight * Sizeof.FLOAT);
	...
	public void compute(final float[] scores, final float[] whiteObservations, final float[] blackObservations, final int sequenceLength) {
		...
		cuMemcpyHtoD(d_Ow, Pointer.to(CudaUtil.extendWithZeros(whiteObservations, (extendedSeqLength+maxTemplateWidth-1)*lineHeight)), (extendedSeqLength+maxTemplateWidth-1)*lineHeight * Sizeof.FLOAT);
		cuMemcpyHtoD(d_Ob, Pointer.to(CudaUtil.extendWithZeros(blackObservations, (extendedSeqLength+maxTemplateWidth-1)*lineHeight)), (extendedSeqLength+maxTemplateWidth-1)*lineHeight * Sizeof.FLOAT);
		for (int tw=minTemplateWidth; tw<=maxTemplateWidth; ++tw) {
			if (templateNumIndices[tw-minTemplateWidth] > 0) {
    ...
    public String kernelSrcShared() {
    	assert BLOCK_SIZE_X * BLOCK_SIZE_Y >= (BLOCK_SIZE_X*ROLL_X+(lineHeight-1));
    	StringBuffer buf = new StringBuffer();
    	for (int tw=1; tw<=lineHeight; ++tw) {
    		buf.append("extern \"C\"\n");
    		...
    		buf.append("__shared__ float sO["+(BLOCK_SIZE_X*ROLL_X+(tw-1))*lineHeight+"];\n");
    		buf.append("int sharedIndex = threadIdx.x * "+BLOCK_SIZE_Y+" + threadIdx.y;\n");
    		buf.append("if (sharedIndex < "+(BLOCK_SIZE_X*ROLL_X+(tw-1))+") {\n");
    		buf.append("for (int i=0; i<"+lineHeight+"; ++i) {\n");
    		buf.append("sO[sharedIndex * "+lineHeight+" + i] = Ow[(blockIdx.x * "+BLOCK_SIZE_X*ROLL_X+" + sharedIndex) * "+lineHeight+" + i];\n");
    		buf.append("}\n");
    		buf.append("}\n");
    		...
    		buf.append("for (int i=0; i<"+lineHeight*tw+"; ++i) {\n");
    		buf.append("float tw = Tw[Tindex * "+lineHeight*tw+" + i];\n");
    		for (int r=0; r<ROLL_X; ++r) {
    			buf.append("score"+r+" = __fmaf_ru(sO[(threadIdx.x * "+ROLL_X+" + "+r+") * "+lineHeight+" + i], tw, score"+r+");\n");
    		}
    		buf.append("}\n");
    		...
    		buf.append("if (sharedIndex < "+(BLOCK_SIZE_X*ROLL_X+(tw-1))+") {\n");
    		buf.append("for (int i=0; i<"+lineHeight+"; ++i) {\n");
    		buf.append("sO[sharedIndex * "+lineHeight+" + i] = Ob[(blockIdx.x * "+BLOCK_SIZE_X*ROLL_X+" + sharedIndex) * "+lineHeight+" + i];\n");
    		buf.append("}\n");
    		buf.append("}\n");
    		buf.append("__syncthreads();\n");
    		...
    		buf.append("if (Tindex < Tlength) {\n");
    		buf.append("for (int i=0; i<"+lineHeight*tw+"; ++i) {\n");
    		buf.append("float tb = Tb[Tindex * "+lineHeight*tw+" + i];\n");
    		for (int r=0; r<ROLL_X; ++r) {
    			buf.append("score"+r+" = __fmaf_ru(sO[(threadIdx.x * "+ROLL_X+" + "+r+") * "+lineHeight+" + i], tb, score"+r+");\n");
    		}
    		buf.append("}\n");
    ...
    public String kernelSrcPrivate() {
    	...
    	for (int tw=1; tw<=lineHeight; ++tw) {
    		...
    		buf.append("float pO["+(ROLL_X+(tw-1))*lineHeight+"];\n");
    		buf.append("for (int r=0; r<"+(ROLL_X+(tw-1))+"; ++r) {\n");
    		buf.append("for (int i=0; i<"+lineHeight+"; ++i) {\n");
    			buf.append("pO[r * "+lineHeight+" + i] = Ow[(blockIdx.x * "+BLOCK_SIZE_X*ROLL_X+" + threadIdx.x * "+ROLL_X+" + r) * "+lineHeight+" + i];\n");
    		buf.append("}\n");
    		buf.append("}\n");
    		...
    		buf.append("float tw = Tw[Tindex * "+lineHeight*tw+" + i];\n");
    		for (int r=0; r<ROLL_X; ++r) {
    			buf.append("score"+r+" = __fmaf_ru(pO["+(r*lineHeight)+" + i], tw, score"+r+");\n");
    		}
    		buf.append("}\n");

    		buf.append("for (int r=0; r<"+(ROLL_X+(tw-1))+"; ++r) {\n");
    		buf.append("for (int i=0; i<"+lineHeight+"; ++i) {\n");
    			buf.append("pO[r * "+lineHeight+" + i] = Ob[(blockIdx.x * "+BLOCK_SIZE_X*ROLL_X+" + threadIdx.x * "+ROLL_X+" + r) * "+lineHeight+" + i];\n");
    		buf.append("}\n");
    		buf.append("}\n");
    		...
    		buf.append("float tb = Tb[Tindex * "+lineHeight*tw+" + i];\n");
    		for (int r=0; r<ROLL_X; ++r) {
    			buf.append("score"+r+" = __fmaf_ru(pO["+(r*lineHeight)+" + i], tb, score"+r+");\n");
    		}
    		buf.append("}\n");
    		buf.append("int Oindex;\n");
    		for (int r=0; r<ROLL_X; ++r) {
