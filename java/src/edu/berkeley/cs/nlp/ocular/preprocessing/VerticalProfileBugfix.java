...
import edu.berkeley.cs.nlp.ocular.preprocessing.VerticalModelBugfix.SuffStats;
import edu.berkeley.cs.nlp.ocular.preprocessing.VerticalModelBugfix.VerticalModelStateType;
 ...
 */
public class VerticalProfileBugfix {
    ...
    private final int[] minSizes3 = { 3, 3, 3 };
    private final int[] minSizes6 = { 6, 6, 6 };
    private final int[] minSizes20 = { 20, 20, 20 };
    private final int[] minSizes50 = { 50, 50, 50 };

    private final int[] maxSizes15 = { 15, 15, 15 };
    private final int[] maxSizes20 = { 20, 20, 20 };
    private final int[] maxSizes30 = { 30, 30, 30 };
    private final int[] maxSizes40 = { 40, 40, 40 };
    private final int[] maxSizes50 = { 50, 50, 50 };
    private final int[] maxSizes60 = { 60, 60, 60 };
    private final int[] maxSizes100 = { 100, 100, 100 };
    private final int[] maxSizes250 = { 250, 250, 250 };

    private final int[] minSizes;
    private final int[] maxSizes;

	public VerticalProfileBugfix(double[][] image, int lineHeight) {
		this.image = image;
		this.emissionsPerRow = new double[image[0].length];
		this.lineHeight = lineHeight;
		if (lineHeight > 100) {
		    this.minSizes = minSizes20;
		    this.maxSizes = maxSizes250;
		} else if (lineHeight > 60) {
		    this.minSizes = minSizes6;
		    this.maxSizes = maxSizes100;
		} else if (lineHeight > 50) {
		    this.minSizes = minSizes3;
		    this.maxSizes = maxSizes60;
		} else if (lineHeight > 40) {
		    this.minSizes = minSizes3;
		    this.maxSizes = maxSizes50;
		} else if (lineHeight > 30) {
		    this.minSizes = minSizes3;
		    this.maxSizes = maxSizes40;
		} else if (lineHeight > 20) {
		    this.minSizes = minSizes3;
		    this.maxSizes = maxSizes30;
		} else if (lineHeight > 15) {
		    this.minSizes = minSizes3;
		    this.maxSizes = maxSizes20;
		} else {
		    this.minSizes = minSizes3;
		    this.maxSizes = maxSizes15;
		}
	...
	public static interface EMCallback {
		public void callback(VerticalModelBugfix model, VerticalProfileBugfix profile);
	}

	public VerticalModelBugfix runEM(int numItrs, int numRestarts) {
		return runEM(numItrs, numRestarts, null);
	}

	public VerticalModelBugfix runEM(int numItrs, int numRestarts, EMCallback callback) {
		double bestLogProb = Double.NEGATIVE_INFINITY;
		VerticalModelBugfix bestModel = null;
		Random rand = new Random();
		for (int r=0; r<numRestarts; ++r) {
				System.out.println("EM RESTART " + (r + 1) + " (of " + numRestarts + " with " + numItrs + " iterations each, min = " + minSizes[0] + " max = " + maxSizes[0] + ")");
			VerticalModelBugfix model = VerticalModelBugfix.getRandomlyInitializedModel(image.length, rand, minSizes, maxSizes);
...
//				System.out.println("END OF ITERATION " + itr + ": NORMALIZER = " + logNormalizer);
			...
			if (logNormalizer > bestLogProb) {
				...
				bestModel = model;
				System.out.println("NEW BEST MODEL NORMALIZER = " + logNormalizer);
			}
		}
		return bestModel;
	}

	public VerticalSegmentation decode(VerticalModelBugfix model) {
	...
	private double[][] computeAlphas(VerticalModelBugfix model, boolean max) {
	...
	private double[][] computeBetas(VerticalModelBugfix model, boolean max) {
		int len = emissionsPerRow.length + 1;
		int numStates = model.numStates();
