...
import edu.berkeley.cs.nlp.ocular.preprocessing.VerticalProfileBugfix.VerticalSegmentation;
import tuple.Pair;
import fileio.f;
 ...
 */
public class LineExtractorBugfix {
	
	public static List<double[][]> extractLines(double[][] levels, int numExtractIterations, int numExtractRestarts, int lineHeight) {
		VerticalProfileBugfix verticalProfile = new VerticalProfileBugfix(levels, lineHeight);
		// 5, 100
		VerticalModelBugfix trainedModel = verticalProfile.runEM(numExtractIterations, numExtractRestarts);
...
//			ImageUtils.display(ImageUtils.makeImage(cropLevels));
			List<double[][]> lines = extractLines(cropLevels, 5, 100, 100);
