 ...
 */
public class OpenCLInnerLoopRC4 implements EmissionCacheInnerLoopVarLineHeight {
	...
	public OpenCLInnerLoopRC4(int numThreads) {
		// choose device
...
//	    }
	...
	public void startup(float[][] whiteTemplates, float[][] blackTemplates, int[] templateNumIndices, int[] templateIndicesOffsets, int minTemplateWidth, int maxTemplateWidth, int maxSequenceLength, int totalTemplateNumIndices, int lineHeight) {
		...
		this.d_Ow = context.createFloatBuffer(Usage.Input, (extendedMaxSeqLength+maxTemplateWidth-1)*lineHeight);
		this.d_Ob = context.createFloatBuffer(Usage.Input, (extendedMaxSeqLength+maxTemplateWidth-1)*lineHeight);
	...
	public void compute(final float[] scores, final float[] whiteObservations, final float[] blackObservations, final int sequenceLength) {
		...
		d_Ow.write(queue, pc.capture(Pointer.pointerToFloats(CudaUtil.extendWithZeros(whiteObservations, (extendedSeqLength+maxTemplateWidth-1)*lineHeight))), false);
		d_Ob.write(queue, pc.capture(Pointer.pointerToFloats(CudaUtil.extendWithZeros(blackObservations, (extendedSeqLength+maxTemplateWidth-1)*lineHeight))), false);
		queue.enqueueBarrier(CLEvent.FIRE_AND_FORGET);
		for (int tw=minTemplateWidth; tw<=maxTemplateWidth; ++tw) {
			if (templateNumIndices[tw-minTemplateWidth] > 0) {
    ...
    public String kernelSrc() {
    	...
    	for (int tw=1; tw<=lineHeight; ++tw) {
    		...
    		for (int r=0; r<rollX; ++r) {
    			buf.append("o"+r+" = Ow[(get_group_id(0) * "+blockSizeX*rollX+" + get_local_id(0) * "+rollX+" + "+r+") * "+lineHeight+" + i];\n");
...
//    			buf.append("score"+r+" = mad(o"+r+", tw, score"+r+");\n");
    		...
    		for (int r=0; r<rollX; ++r) {
    			buf.append("o"+r+" = Ob[(get_group_id(0) * "+blockSizeX*rollX+" + get_local_id(0) * "+rollX+" + "+r+") * "+lineHeight+" + i];\n");
