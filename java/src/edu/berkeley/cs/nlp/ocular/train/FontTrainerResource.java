...
import static edu.berkeley.cs.nlp.ocular.eval.EvalPrinterResource.printEvaluation;
 ...
 */
public class FontTrainerResource {
	
	public Tuple3<FontVarLineHeight, CodeSwitchLanguageModel, GlyphSubstitutionModel> trainFont(
				List<Document> inputDocuments,  
				FontVarLineHeight font, CodeSwitchLanguageModel lm, GlyphSubstitutionModel gsm,
				TrainingRestarterResource trainingRestarter,
				...
				MultiDocumentTranscriberVarLineHeight evalSetIterationEvaluator, int evalFreq, boolean evalBatches) {
		...
		Tuple2<Integer, Tuple3<FontVarLineHeight, CodeSwitchLanguageModel, GlyphSubstitutionModel>> restartObjects;
		...
		for (int iter = lastCompletedIteration+1; (/* trainFont && */ iter <= numEMIters) || (/* !trainFont && */ iter == 1); ++iter) {
			...
			Tuple3<FontVarLineHeight, CodeSwitchLanguageModel, GlyphSubstitutionModel> iterationResultModels = 
	...
	public Tuple3<FontVarLineHeight, CodeSwitchLanguageModel, GlyphSubstitutionModel> doFontTrainPass(
			int iter,
			List<Document> inputDocuments,  
			FontVarLineHeight font, CodeSwitchLanguageModel lm, GlyphSubstitutionModel gsm,
			String outputFontPath, String outputLmPath, String outputGsmPath,
		...
		final CharacterTemplateVarLineHeight[] templates = loadTemplates(font, charIndexer);
	...
	public static CharacterTemplateVarLineHeight[] loadTemplates(FontVarLineHeight font, Indexer<String> charIndexer) {
		...
		for (int c = 0; c < charIndexer.size(); ++c) {
			CharacterTemplateVarLineHeight template = font.get(charIndexer.getObject(c));
			if (template == null)
	...
	private void clearTemplates(final CharacterTemplateVarLineHeight[] templates) {
	...
	private void updateFontParameters(final CharacterTemplateVarLineHeight[] templates, int numMstepThreads) {
