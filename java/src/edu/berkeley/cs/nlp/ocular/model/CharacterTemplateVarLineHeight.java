 ...
 */
public class CharacterTemplateVarLineHeight implements Serializable {
	...
	public CharacterTemplateVarLineHeight(String character, float templateMaxWidthFraction, float templateMinWidthFraction, int lineHeight) {
	    this.lineHeight = lineHeight;
		this.templateMaxWidth = (int) Math.max(1, Math.floor(templateMaxWidthFraction*lineHeight));
		this.templateMinWidth = (int) Math.max(1, Math.floor(templateMinWidthFraction*lineHeight));
		...
		if (!character.equals(Charset.SPACE)) {
			this.templateWeights = new float[templateMaxWidth][lineHeight];
			for (int i=0; i<templateMaxWidth; ++i) {
				Arrays.fill(templateWeights[i], 0.0f);
			};

			this.templateWeightsPriorMeans = new float[templateMaxWidth][lineHeight];
			for (int i=0; i<templateMaxWidth; ++i) {
			...
			for (int e=0; e<EXP_GAINS.length; ++e) {
				for (int w=0; w<templateWidthProbs.length; ++w) {
					...
					this.templateLogBlackProbs[e][w] = new float[width][lineHeight];
					this.templateLogWhiteProbs[e][w] = new float[width][lineHeight];
					this.templateBlackCounts[e][w] = new float[width][lineHeight];
					this.templateWhiteCounts[e][w] = new float[width][lineHeight];
	...
	public void initializeAndSetPriorFromFontData(PixelType[][][] fontData) {
		if (!character.equals(Charset.SPACE)) {
			System.out.println("Initializing '" + Charset.unescapeChar(character) + "' from font data...");
			clearEmissionCounts();
			clearWidthCounts();
	...
	public float[][] blackProbs(int exposure, int offset, int width) {
		float[][] result = new float[width][lineHeight];
		if (!character.equals(Charset.SPACE)) {
			for (int i=0; i<width; ++i) {
				for (int j=0; j<lineHeight; ++j) {
					result[i][j] = (float) Math.exp(templateLogProbs(width, exposure, true)[i][Math.min(lineHeight-1, Math.max(0, j+offset))]);
				}
			}
		} else {
			for (int i=0; i<width; ++i) {
	...
	public float[][] logBlackProbs(int exposure, int offset, int width) {
		...
		if (!character.equals(Charset.SPACE)) {
			for (int i=0; i<width; ++i) {
				for (int j=0; j<lineHeight; ++j) {
					result[i][j] = (float) templateLogProbs(width, exposure, true)[i][Math.min(lineHeight-1, Math.max(0, j+offset))];
				}
			}
		} else {
			for (int i=0; i<width; ++i) {
	...
	public float[][] logWhiteProbs(int exposure, int offset, int width) {
		...
		if (!character.equals(Charset.SPACE)) {
			for (int i=0; i<width; ++i) {
				for (int j=0; j<lineHeight; ++j) {
					result[i][j] = (float) templateLogProbs(width, exposure, false)[i][Math.min(lineHeight-1, Math.max(0, j+offset))];
				}
			}
		} else {
			for (int i=0; i<width; ++i) {
	...
	private float pixelEmissionLogProb(int exposure, int offset, int width, int pos, int j, PixelType observation) {
		if (!character.equals(Charset.SPACE)) {
			if (observation == PixelType.BLACK) {
				return templateLogProbs(width, exposure, true)[pos][Math.min(lineHeight-1, Math.max(0, j+offset))];
			} if (observation == PixelType.WHITE) {
				return templateLogProbs(width, exposure, false)[pos][Math.min(lineHeight-1, Math.max(0, j+offset))];
			} else {
				return 0.0f;
			} 
		} else {
	...
	public String toString() {
		...
		buf.append(Charset.unescapeCharPrecomposedOnly(character)).append(":\n");
		for (int j=0; j<lineHeight; ++j) {
			...
			synchronized (templateBlackCounts[exposure][width-templateMinWidth()][pos]) {
				for (int j=0; j<observation.length; ++j) {
					if (observation[j] == PixelType.BLACK) {
						templateBlackCounts[exposure][width-templateMinWidth()][pos][Math.min(lineHeight-1, Math.max(0, j+offset))] += count;
					} else if (observation[j] == PixelType.WHITE) {
						templateWhiteCounts[exposure][width-templateMinWidth()][pos][Math.min(lineHeight-1, Math.max(0, j+offset))] += count;
					}
				}
			}
