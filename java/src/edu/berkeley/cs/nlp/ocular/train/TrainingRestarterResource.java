 ...
 */
public class TrainingRestarterResource {

	/**
	 * If requested, try and pick up where we left off
	 */
	public Tuple2<Integer, Tuple3<FontVarLineHeight, CodeSwitchLanguageModel, GlyphSubstitutionModel>> getRestartModels(
			FontVarLineHeight inputFont, CodeSwitchLanguageModel inputLm, GlyphSubstitutionModel inputGsm, 
		...
		for (int iter = 1; iter <= numEMIters; ++iter) {
			...
			if (new Resource(fontPath).exists()) {
				lastCompletedIteration = iter;
			}
		}
		
		FontVarLineHeight newFont = inputFont;
	...
	private int getLastBatchNumOfIteration(int numUsableDocs, int updateDocBatchSize, boolean noUpdateIfBatchTooSmall) {
		...
		for (int docNum = 0; docNum < numUsableDocs; ++docNum) {
			++currentBatchSize;
			if (FontTrainerResource.isBatchComplete(numUsableDocs, docNum, currentBatchSize, updateDocBatchSize, noUpdateIfBatchTooSmall)) {
