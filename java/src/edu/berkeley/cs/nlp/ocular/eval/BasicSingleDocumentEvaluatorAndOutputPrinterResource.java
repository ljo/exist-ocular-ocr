...
import edu.berkeley.cs.nlp.ocular.output.AltoOutputWriterResource;
import edu.berkeley.cs.nlp.ocular.output.HocrOutputWriterResource;
import edu.berkeley.cs.nlp.ocular.util.ArrayHelper;
import edu.berkeley.cs.nlp.ocular.util.CollectionHelper;
import edu.berkeley.cs.nlp.ocular.util.FileHelperResource;
 ...
 */
public class BasicSingleDocumentEvaluatorAndOutputPrinterResource implements SingleDocumentEvaluatorAndOutputPrinter {
	private Indexer<String> charIndexer;
	private Indexer<String> langIndexer;
	private boolean allowGlyphSubstitution;
	private boolean charIncludesDiacritic;
	
	public BasicSingleDocumentEvaluatorAndOutputPrinterResource(Indexer<String> charIndexer, Indexer<String> langIndexer, boolean allowGlyphSubstitution, boolean charIncludesDiacritic) {
	...
	public Tuple2<Map<String, EvalSuffStats>,Map<String, EvalSuffStats>> evaluateAndPrintTranscription(int iter, int batchId,
		...
		new Resource(outputFilenameBase).getParentFile().mkdirs();
		...
		//
		{
			...
			FileHelperResource.writeString(transcriptionOutputFilename, transcriptionOutputBuffer.toString());
		}
		...
		if (allowGlyphSubstitution || goldDiplomaticLineChars != null || goldNormalizedLineChars != null) {
			...
			fResource.writeString(transcriptionOutputFilename, goldComparisonOutputBuffer.toString());
		}
		
		//
		// Other files
		//
		new AltoOutputWriterResource(charIndexer, langIndexer).write(numLines, mt.viterbiTransStates, inputDocPath, outputFilenameBase, mt.viterbiWidths);
		new HocrOutputWriterResource(charIndexer, langIndexer).write(numLines, mt.viterbiTransStates, inputDocPath, outputFilenameBase);
...
//		FileHelperResource.writeString(transcriptionWithWidthsOutputFilename, transcriptionWithWidthsOutputBuffer.toString());
//		}
