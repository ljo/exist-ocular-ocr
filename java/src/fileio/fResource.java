...
public class fResource {
	...
	try {
	    Resource outputfile = new Resource(path);
	    if (!outputfile.exists()) outputfile.createNewFile();
	    OutputStream os = outputfile.getOutputStream();
	    ImageIO.write(image, path.substring(path.lastIndexOf(".") + 1), os);
	    os.flush();
	    os.close();
	} catch (IOException e) {
	...
	try {
	    InputStream is = new Resource(path).getInputStream();
	...
	try {
	    ...
	    BufferedReader in = new Resource(path).getBufferedReader();
	    //while (in.ready()) {
	    String line = null;
