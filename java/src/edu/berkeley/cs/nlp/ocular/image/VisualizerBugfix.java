...
public class VisualizerBugfix {
	...
	private static int[][] combineLinesRGB(int[][][] imageByLine) {
		...
		int[] varLinesHeight = new int[imageByLine.length + 1];
		int varLinesHeightSum = 0;
		varLinesHeight[0] = 0;
		for (int d=0; d<imageByLine.length; ++d) {
			longestLineLength = Math.max(longestLineLength, imageByLine[d].length);

			varLinesHeightSum += imageByLine[d][0].length; 
			varLinesHeight[d + 1] = varLinesHeightSum;
		}
		int[][] combinedLines = new int[longestLineLength][varLinesHeightSum];
		for (int d=0; d<imageByLine.length; ++d) {
			for (int w=0; w<combinedLines.length; ++w) {
			    for (int h=0; h< imageByLine[d][0].length; ++h) {
					// If we're still writing the line
					if (w<imageByLine[d].length) {
						combinedLines[w][h+varLinesHeight[d]] = imageByLine[d][w][h];
					} else {
						combinedLines[w][h+varLinesHeight[d]] = Color.LIGHT_GRAY.getRGB();
					}
				}
			}
		}
