...
import edu.berkeley.cs.nlp.ocular.train.TrainingRestarterResource;
...
public class TrainFont extends BasicFunction {

    public final static FunctionSignature signatures[] = {
	new FunctionSignature(
			      new QName("train-font", OcularOCRModule.NAMESPACE_URI, OcularOCRModule.PREFIX),
			      "Train a font from the language model, initialized font and input documents. Returns the path to the trained font document " +
				  ...
				  new FunctionParameterSequenceType("init-font-uri", Type.ANY_URI, Cardinality.EXACTLY_ONE,
				  ...
				  new FunctionParameterSequenceType("output-path-uri", Type.ANY_URI, Cardinality.ZERO_OR_ONE,
				  ...
				  new FunctionParameterSequenceType("trained-font-uri", Type.ANY_URI, Cardinality.EXACTLY_ONE,
								    "The path to the serialized trained font based on the language model, initialized font and input document(s)"),
                                  new FunctionParameterSequenceType("configuration", Type.ELEMENT, Cardinality.EXACTLY_ONE,
                                                                    "The configuration, eg &lt;parameters&gt;&lt;param name='numEMIters' value='3'/&gt;&lt;param name='updateDocBatchSize' value='-1'/&gt;&lt;/parameters&gt;.")
			      },
			      new FunctionReturnSequenceType(Type.ANY_URI, Cardinality.ZERO_OR_ONE,
							     "The path to the stored trained font")			      
			      )
    };
    ...
    private boolean learnFont = false;
    ...
    private boolean continueFromLastCompleteIteration = false;
    ...
    private int evalFreq = Integer.MAX_VALUE;
    ...
    public TrainFont(XQueryContext context, FunctionSignature signature) {
	...
	if (!args[4].isEmpty()) {
	    outputFontPath = args[4].getStringValue();
	}
	...
	CodeSwitchLanguageModel initialLM = InitializeLanguageModel.readCodeSwitchLM(lmPath);
	...
	List<String> inputDocPathList = getInputDocPathList(inputPath, inputDocListPath);
	...
	MultiDocumentTranscriberVarLineHeight evalSetEvaluator = FonttrainTranscribeSharedResource.makeEvalSetEvaluator(charIndexer, decoderEM, documentOutputPrinterAndEvaluator, evalInputDocPath, evalExtractedLinesPath, evalNumDocs, uniformLineHeight, lineHeight, binarizeThreshold, crop, numExtractIterations, numExtractRestarts, extractLineHeight, outputPath);
		new FontTrainerResource().trainFont(
				inputDocuments,  
				initialFont, initialLM, initialGSM,
				continueFromLastCompleteIteration ? new TrainingRestarterResource() : null,
	...
	try {
        ...
        if (isCalledAs("train-font")) {
	    return new StringValue(outputFontPath);
	}
	return Sequence.EMPTY_SEQUENCE;
    }

    protected static List<String> getInputDocPathList(final String inputPath, final String inputDocListPath) {
                //return inputDocPath != null ? Arrays.asList(inputDocPath.split("[\\s+,;:]")) : fResource.readLines(inputDocListPath);
return inputPath != null ? Arrays.asList(inputPath.split("[\\s+,;:]")) : fResource.readLines(inputDocListPath);
        }
